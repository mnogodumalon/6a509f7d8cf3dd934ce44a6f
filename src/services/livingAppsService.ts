...
import type { Objektverwaltung, Maengelverwaltung, CreateObjektverwaltung, CreateMaengelverwaltung } from '@/types/app';
...
export class LivingAppsService {
  // --- OBJEKTVERWALTUNG ---
  static async getObjektverwaltung(): Promise<Objektverwaltung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.OBJEKTVERWALTUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Objektverwaltung[];
    return enrichLookupFields(records, 'objektverwaltung');
  }
  static async getObjektverwaltungEntry(id: string): Promise<Objektverwaltung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.OBJEKTVERWALTUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Objektverwaltung;
    return enrichLookupFields([record], 'objektverwaltung')[0];
  }
  static async createObjektverwaltungEntry(fields: CreateObjektverwaltung) {
    return callApi('POST', `/apps/${APP_IDS.OBJEKTVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'objektverwaltung') });
  }
  static async updateObjektverwaltungEntry(id: string, fields: Partial<CreateObjektverwaltung>) {
    return callApi('PATCH', `/apps/${APP_IDS.OBJEKTVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'objektverwaltung') });
  }
  static async deleteObjektverwaltungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.OBJEKTVERWALTUNG}/records/${id}`);
  }

  // --- MAENGELVERWALTUNG ---
  static async getMaengelverwaltung(): Promise<Maengelverwaltung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MAENGELVERWALTUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Maengelverwaltung[];
    return enrichLookupFields(records, 'maengelverwaltung');
  }
  static async getMaengelverwaltungEntry(id: string): Promise<Maengelverwaltung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.MAENGELVERWALTUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Maengelverwaltung;
    return enrichLookupFields([record], 'maengelverwaltung')[0];
  }
  static async createMaengelverwaltungEntry(fields: CreateMaengelverwaltung) {
    return callApi('POST', `/apps/${APP_IDS.MAENGELVERWALTUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'maengelverwaltung') });
  }
  static async updateMaengelverwaltungEntry(id: string, fields: Partial<CreateMaengelverwaltung>) {
    return callApi('PATCH', `/apps/${APP_IDS.MAENGELVERWALTUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'maengelverwaltung') });
  }
  static async deleteMaengelverwaltungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.MAENGELVERWALTUNG}/records/${id}`);
  }
