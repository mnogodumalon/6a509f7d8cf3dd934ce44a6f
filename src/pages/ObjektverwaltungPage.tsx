...
export default function ObjektverwaltungPage() {
  ...
  const [records, setRecords] = useState<Objektverwaltung[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Objektverwaltung | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Objektverwaltung | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getObjektverwaltung());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Objektverwaltung['fields']) {
  ...
  async function handleUpdate(fields: Objektverwaltung['fields']) {
    ...
    await LivingAppsService.updateObjektverwaltungEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteObjektverwaltungEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Objektverwaltung im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Objektverwaltung suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('anzahl_einheiten')}>
                <span className="inline-flex items-center gap-1">
                  Anzahl der Wohneinheiten
                  {sortKey === 'anzahl_einheiten' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('vermietungsstand')}>
                <span className="inline-flex items-center gap-1">
                  Vermietungsstand (Anzahl vermieteter Einheiten)
                  {sortKey === 'vermietungsstand' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/objektverwaltung/${record.record_id}`); }}>
                ...
                <TableCell>{record.fields.anzahl_einheiten ?? '—'}</TableCell>
                <TableCell>{record.fields.vermietungsstand ?? '—'}</TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={9} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Objektverwaltung. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
