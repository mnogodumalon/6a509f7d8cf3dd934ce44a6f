...
import { ObjektverwaltungViewDialog } from '@/components/dialogs/ObjektverwaltungViewDialog';
import { MaengelverwaltungDialog } from '@/components/dialogs/MaengelverwaltungDialog';
import { MaengelverwaltungViewDialog } from '@/components/dialogs/MaengelverwaltungViewDialog';
...
const OBJEKTVERWALTUNG_FIELDS = [
  ...
  { key: 'anzahl_einheiten', label: 'Anzahl der Wohneinheiten', type: 'number' },
  { key: 'vermietungsstand', label: 'Vermietungsstand (Anzahl vermieteter Einheiten)', type: 'number' },
];
const MAENGELVERWALTUNG_FIELDS = [
  { key: 'objekt', label: 'Objekt', type: 'applookup/select', targetEntity: 'objektverwaltung', targetAppId: 'OBJEKTVERWALTUNG', displayField: 'bezeichnung' },
  { key: 'beschreibung', label: 'Was ist kaputt / Schadensbeschreibung', type: 'string/textarea' },
  { key: 'dringlichkeit', label: 'Dringlichkeit', type: 'lookup/radio', options: [{ key: 'niedrig', label: 'Niedrig' }, { key: 'mittel', label: 'Mittel' }, { key: 'hoch', label: 'Hoch' }, { key: 'dringend', label: 'Dringend' }] },
  ...
  { key: 'handwerker', label: 'Beauftragter Handwerker', type: 'string/text' },
  { key: 'status', label: 'Status', type: 'lookup/radio', options: [{ key: 'offen', label: 'Offen' }, { key: 'beauftragt', label: 'Beauftragt' }, { key: 'erledigt', label: 'Erledigt' }] },
  { key: 'fotos', label: 'Fotos vom Schaden', type: 'file' },
];

const ENTITY_TABS = [
  { key: 'objektverwaltung', label: 'Objektverwaltung', pascal: 'Objektverwaltung' },
  { key: 'maengelverwaltung', label: 'Mängelverwaltung', pascal: 'Maengelverwaltung' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('objektverwaltung');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'objektverwaltung': new Set(),
    'maengelverwaltung': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'objektverwaltung': return (data as any).objektverwaltung as Objektverwaltung[] ?? [];
      case 'maengelverwaltung': return (data as any).maengelverwaltung as Maengelverwaltung[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'maengelverwaltung':
        lists.objektverwaltungList = (data as any).objektverwaltung ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'maengelverwaltung' && fieldKey === 'objekt') {
      const match = (lists.objektverwaltungList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'objektverwaltung': return OBJEKTVERWALTUNG_FIELDS;
      case 'maengelverwaltung': return MAENGELVERWALTUNG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'objektverwaltung': return {
        create: (fields: any) => LivingAppsService.createObjektverwaltungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateObjektverwaltungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteObjektverwaltungEntry(id),
      };
      case 'maengelverwaltung': return {
        create: (fields: any) => LivingAppsService.createMaengelverwaltungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateMaengelverwaltungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteMaengelverwaltungEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'objektverwaltung' || dialogState?.entity === 'objektverwaltung') && (
        <ObjektverwaltungDialog
          open={createEntity === 'objektverwaltung' || dialogState?.entity === 'objektverwaltung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'objektverwaltung' ? handleUpdate : (fields: any) => handleCreate('objektverwaltung', fields)}
          defaultValues={dialogState?.entity === 'objektverwaltung' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Objektverwaltung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Objektverwaltung']}
        />
      )}
      {(createEntity === 'maengelverwaltung' || dialogState?.entity === 'maengelverwaltung') && (
        <MaengelverwaltungDialog
          open={createEntity === 'maengelverwaltung' || dialogState?.entity === 'maengelverwaltung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'maengelverwaltung' ? handleUpdate : (fields: any) => handleCreate('maengelverwaltung', fields)}
          defaultValues={dialogState?.entity === 'maengelverwaltung' ? dialogState.record?.fields : undefined}
          objektverwaltungList={(data as any).objektverwaltung ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Maengelverwaltung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Maengelverwaltung']}
        />
      )}
      {viewState?.entity === 'objektverwaltung' && (
        <ObjektverwaltungViewDialog
          open={viewState?.entity === 'objektverwaltung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'objektverwaltung', record: r }); }}
        />
      )}
      {viewState?.entity === 'maengelverwaltung' && (
        <MaengelverwaltungViewDialog
          open={viewState?.entity === 'maengelverwaltung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'maengelverwaltung', record: r }); }}
          objektverwaltungList={(data as any).objektverwaltung ?? []}
        />
      )}
