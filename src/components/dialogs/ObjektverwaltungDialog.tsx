...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Objektverwaltung';
...
interface ObjektverwaltungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Objektverwaltung['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Objektverwaltung['fields'];
...
export function ObjektverwaltungDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: ObjektverwaltungDialogProps) {
  const [fields, setFields] = useState<Partial<Objektverwaltung['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['bezeichnung', 'strasse', 'hausnummer', 'plz', 'stadt', 'anzahl_einheiten'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Objektverwaltung['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'objektverwaltung');
      await onSubmit(clean as Objektverwaltung['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Objektverwaltung['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Objektverwaltung bearbeiten' : 'Objektverwaltung hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'bezeichnung': (
      <div key="bezeichnung" className="space-y-1.5">
        ...
        <Input
          id="bezeichnung"
          placeholder="z. B. Mietshaus Hauptstraße"
    ...
    'anzahl_einheiten': (
      <div key="anzahl_einheiten" className="space-y-1.5">
        <Label htmlFor="anzahl_einheiten">Anzahl der Wohneinheiten <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="anzahl_einheiten"
          type="number"
          step="any"
    ...
    'vermietungsstand': (
      <div key="vermietungsstand" className="space-y-1.5">
        <Label htmlFor="vermietungsstand">Vermietungsstand (Anzahl vermieteter Einheiten)</Label>
        <Input
          id="vermietungsstand"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'vermietungsstand')}
          placeholder="z. B. 7"
          value={fields.vermietungsstand !== undefined ? fields.vermietungsstand : (computedValues['vermietungsstand'] ?? '')}
          onChange={e => setFields(f => ({ ...f, vermietungsstand: clampNumberValue(formEnhancements, 'vermietungsstand', e.target.value) }))}
        />
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"bezeichnung": "Bezeichnung", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "stadt": "Stadt", "baujahr": "Baujahr", "anzahl_einheiten": "Anzahl der Wohneinheiten", "vermietungsstand": "Vermietungsstand (Anzahl vermieteter Einheiten)"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.OBJEKTVERWALTUNG} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
